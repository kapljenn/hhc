...
class MM_WPFS {
	...
	const HANDLE_STRIPE_CHECKOUT_JS = 'stripe-checkout-js';
	...
	public static function get_rendered_forms() {
		if ( ! array_key_exists( self::REQUEST_PARAM_NAME_WPFS_RENDERED_FORMS, $_REQUEST ) ) {
			$_REQUEST[ self::REQUEST_PARAM_NAME_WPFS_RENDERED_FORMS ] = new RenderedFormData();
		}
	...
	function fullstripe_payment_form( $atts ) {
		...
		if ( ! $formData ) {
			...
			$rendered_forms = self::get_rendered_forms()->render_later( RenderedFormData::FORM_TYPE_PAYMENT );
			if ( $rendered_forms->get_payments() + $rendered_forms->get_subscriptions() == 1 ) {
				$this->fullstripe_load_js();
			}
			if ( $rendered_forms->get_total() == 1 ) {
				$this->fullstripe_load_css();
				if ( $rendered_forms->get_payments() + $rendered_forms->get_subscriptions() == 1 ) {
					$this->fullstripe_set_common_js_variables( self::HANDLE_STRIPE_JS );
				}
			}
	...
	function fullstripe_subscription_form( $atts ) {
		...
		if ( ! $formData ) {
			...
			if ( count( $allPlans ) === 0 ) {
				...
				// tnagy load css and js only once per request
				$rendered_forms = self::get_rendered_forms()->render_later( RenderedFormData::FORM_TYPE_SUBSCRIPTION );
				if ( $rendered_forms->get_payments() + $rendered_forms->get_subscriptions() == 1 ) {
					$this->fullstripe_load_js();
				}
				if ( $rendered_forms->get_total() == 1 ) {
	...
	function fullstripe_checkout_form( $atts ) {
		...
		if ( ! $formData ) {
			...
			// tnagy load css and js only once per request
			$rendered_forms = self::get_rendered_forms()->render_later( RenderedFormData::FORM_TYPE_CHECKOUT );
			if ( $rendered_forms->get_checkouts() == 1 ) {
				$this->fullstripe_load_checkout_js();
			}
			if ( $rendered_forms->get_total() == 1 ) {
				$this->fullstripe_load_css();
				if ( $rendered_forms->get_checkouts() == 1 ) {
					$this->fullstripe_set_common_js_variables( self::HANDLE_STRIPE_CHECKOUT_JS );
				}
			}
	...
	function fullstripe_load_checkout_js() {
		...
		wp_enqueue_script( self::HANDLE_STRIPE_CHECKOUT_JS, plugins_url( 'js/wp-full-stripe-checkout.js', dirname( __FILE__ ) ), array(
			'jquery',
			'checkout-js'
		), MM_WPFS::VERSION );
	...
	function fullstripe_set_common_js_variables( $handle ) {
		// 'wp-full-stripe-js'
		// 
		wp_localize_script( $handle, 'ajaxurl', admin_url( 'admin-ajax.php' ) );
		$options = get_option( 'fullstripe_options' );
		if ( $options['apiMode'] === 'test' ) {
			wp_localize_script( $handle, 'stripekey', $options['publishKey_test'] );
		} else {
			wp_localize_script( $handle, 'stripekey', $options['publishKey_live'] );
		}

		wp_localize_script( $handle, 'wpfs_L10n', array(
...
class RenderedFormData {
	...
	public function get_total() {
		return $this->payments + $this->subscriptions + $this->checkouts;
	}
